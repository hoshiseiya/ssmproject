...
//        （3）通过线索对象提取联系人信息，保存联系人(经过第二步之后客户对象已经有了，将来在处理其他表的时候，如果要使用客户的id，直接使用cus.id就可以了)
        ...
        //查询出与该线索关联的备注信息列表
        List<ClueRemark> clueRemarkList = clueDao.getListByClueId(clueId);
        //取出每一条线索的备注
...
//            创建客户备注对象，添加客户备注
            ...
            int Count3 = customerDao.saveRemark(customerRemark);
            if (Count3 != 1) {
                flag = false;
            }
            //创建客户备注对象，添加客户备注
            ...
            int Count4 = contactsDao.saveRemark(contactsRemark);
            if (Count4 != 1) {
                flag = false;
            }
        }
            ...
            for (ClueRemark clueRemark : clueRemarkList) {
                ...
                int Count8 = clueDao.deleteRemark(clueRemark);
                if (Count8 != 1) {
                    flag = false;
                }
            }
