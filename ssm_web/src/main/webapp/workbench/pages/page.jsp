...
<div style="position: absolute;top: 350px; left: 5px;width:800px;">
    ...
    <c:choose>
        <c:when test="${page.hasPreviousPage}"><%--如果有上一页，就可以点上一页，否则不可点击--%>
        <li><a href="${url}&pageNum=1">首页</a></li>
        <li><a href="${url}&pageNum=${page.prePage}">上一页</a></li>
        </c:when>
        <c:otherwise>
        <li><a class="disabled" href="${url}&pageNum=1">首页</a></li>
        <li><a class="disabled" href="${url}&pageNum=${page.prePage}">上一页</a></li>
        </c:otherwise>
    </c:choose>
    ...
    <c:choose>
        ...
        <c:when test="${ page.pages <= 5 }">
            ...
            <c:set var="end" value="${page.pages}" scope="page"/>
        </c:when>
        <%--情况2：总页码大于5 的情况--%>
        <c:when test="${page.pages > 5}">
            <c:choose>
    ...
    <c:choose>
        ...
        <li><a href="${url}&pageNum=${page.nextPage}">下一页</a></li>
        <li><a href="${url}&pageNum=${page.pages}">末页</a></li>

        <li>
            <button type="button" class="btn btn-default" style="cursor: default;">共<b>${page.pages}</b>页,<b>${page.total}</b>条记录</button>
        </li>
        ...
        <c:otherwise>
        <li> <a class="disabled" href="${url}&pageNum=${page.nextPage}">下一页</a></li>
        <li> <a class="disabled" href="${url}&pageNum=${page.pages}">末页</a></li>&nbsp;
